...
resource "google_container_cluster" "primary" {
  ...
  master_authorized_networks_config {
    cidr_blocks = {
      ...
      cidr_block   = "${google_compute_instance.gke-bastion.network_interface.0.access_config.0.assigned_nat_ip}/32"
    }
  }
  ...
  lifecycle {
    ignore_changes = ["ip_allocation_policy", "network", "subnetwork"]
  }
}
